...
const App = () => {
  ...
  const [attachments, setAttachments] = useState([]); // අලුතින් එක් කළා
  ...
  // ෆයිල් සිලෙක්ට් කරන ෆන්ක්ෂන් එක
  ...
  const handleSend = async () => {
    ...
    // යුසර් මැසේජ් එකට ෆයිල්ස් ඇඩ් කිරීම
    const userMsg = { 
      role: 'user', 
      content: input,
    ...
    try {
      // මෙතන ඔයාගේ flash මොඩල් එකම පාවිච්චි කළා
      const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" }); 
      let promptParts = [input || "Analyze this file"];
      
      // ෆයිල්ස් තිබේ නම් ඒවා Gemini එකට කියවිය හැකි ලෙස සකස් කිරීම
